...
#2.x items = dict((item, support) for item, support in items.iteritems()
...
#2.x    transaction = filter(lambda v: v in items, transaction)
        transaction = [v for v in transaction if v in items]
        transaction.sort(key=lambda v: items[v], reverse=True)
        return transaction

    master = FPTree()
#2.x for transaction in imap(clean_transaction, transactions):
...
class FPTree(object):
    ...
    def items(self):
        ...
        """
#2.x    for item in self._routes.iterkeys():
...
def basket_fp_growth(transactions, minsup, itemset_size):
    candidate_dct = defaultdict(lambda: 0)
    for itemset, support in find_frequent_itemsets(transactions, minsup, True):
        if len(itemset) >= itemset_size:
            candidate_dct[tuple(itemset)] = support
